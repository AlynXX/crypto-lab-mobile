...
\subsection{Wprowadzenie do podpisów cyfrowych}
Podpis elektroniczny (cyfrowy) to kryptograficzny mechanizm zapewniający:
\begin{itemize}
    \item \textbf{Autentyczność} -- potwierdzenie tożsamości nadawcy,
    \item \textbf{Integralność} -- gwarancja, że dokument nie został zmieniony,
    \item \textbf{Niezaprzeczalność} -- nadawca nie może zaprzeczyć podpisaniu dokumentu.
\end{itemize}

W odróżnieniu od szyfrowania, które zapewnia \textit{poufność}, podpis cyfrowy zapewnia \textit{autentyczność} i \textit{integralność}.
...
\subsection{Dlaczego SHA-256 + RSA?}
Połączenie funkcji skrótu z RSA jest kluczowe z kilku powodów:

\begin{itemize}
    \item \textbf{Wydajność} -- RSA jest wolny dla dużych danych. Zamiast podpisywać cały dokument, podpisujemy tylko 256-bitowy hash.
    \item \textbf{Bezpieczeństwo} -- bezpośrednie podpisywanie danych RSA jest podatne na ataki. Haszowanie eliminuje te problemy.
    \item \textbf{Rozmiar} -- podpis ma stały rozmiar niezależnie od wielkości dokumentu.
    \item \textbf{Standardy} -- RSA-SHA256 to powszechnie uznany standard (PKCS\#1, RFC 8017).
\end{itemize}

\subsection{Format podpisu w CryptoLab}
Podpis w aplikacji ma format:
$$\text{dokument}|\text{hash}|\text{podpis}|\text{klucz\_publiczny}$$

Gdzie:
\begin{itemize}
    \item \textbf{dokument} -- oryginalny tekst (do weryfikacji integralności),
    \item \textbf{hash} -- skrót SHA-256 dokumentu (64 znaki hex),
    \item \textbf{podpis} -- zaszyfrowany hash kluczem prywatnym (hex),
    \item \textbf{klucz\_publiczny} -- para $(n, e)$ do weryfikacji.
\end{itemize}

\subsection{Parametry RSA dla celów edukacyjnych}
Dla demonstracji algorytmu używamy małych liczb pierwszych:
\begin{itemize}
    \item $p = 61$, $q = 53$
    \item $n = p \times q = 3233$
    \item $\phi(n) = (p-1)(q-1) = 3120$
    \item $e = 17$ (wykładnik publiczny)
    \item $d = 2753$ (wykładnik prywatny, $e \cdot d \equiv 1 \pmod{\phi(n)}$)
\end{itemize}

\textbf{Uwaga:} W rzeczywistych zastosowaniach używa się kluczy 2048-4096 bitowych!

\subsection{Bezpieczeństwo podpisu cyfrowego}
Podpis cyfrowy RSA-SHA256 jest bezpieczny pod warunkiem:
\begin{itemize}
    \item \textbf{Klucz prywatny pozostaje tajny} -- tylko właściciel może podpisywać,
    \item \textbf{Silne generowanie kluczy} -- losowe, duże liczby pierwsze,
    \item \textbf{Bezpieczna funkcja skrótu} -- SHA-256 jest odporna na kolizje,
    \item \textbf{Odpowiedni padding} -- w produkcji używa się PSS (Probabilistic Signature Scheme).
\end{itemize}

\subsection{Zastosowania podpisów cyfrowych}
\begin{itemize}
    \item \textbf{Certyfikaty SSL/TLS} -- weryfikacja autentyczności serwerów WWW,
    \item \textbf{Podpisywanie oprogramowania} -- weryfikacja pochodzenia aplikacji,
    \item \textbf{Dokumenty elektroniczne} -- prawnie wiążące podpisy (eIDAS, esignature),
    \item \textbf{Transakcje finansowe} -- autoryzacja przelewów bankowych,
    \item \textbf{Blockchain} -- potwierdzanie transakcji kryptowalut,
    \item \textbf{E-mail} -- S/MIME, PGP do podpisywania wiadomości,
    \item \textbf{Aktualizacje systemu} -- weryfikacja integralności pakietów.
\end{itemize}

\subsection{Implementacja w CryptoLab}
Aplikacja zawiera pełną implementację podpisu elektronicznego:
\begin{itemize}
    \item Generowanie pary kluczy RSA (edukacyjna wersja 12-bitowa),
    \item Integracja z SHA-256 do haszowania dokumentów,
    \item Modular exponentiation dla operacji RSA,
    \item Rozszerzony algorytm Euklidesa dla odwrotności modularnej,
    \item Szczegółowe logowanie każdego kroku podpisywania i weryfikacji,
    \item Wykrywanie modyfikacji dokumentu,
    \item Walidacja podpisu z komunikatami WAŻNY / NIEWAŻNY.
\end{itemize}

\textbf{Demonstracja:}
\begin{enumerate}
    \item Użytkownik wpisuje dokument i klika "Podpisz dokument"
    \item System oblicza hash SHA-256 i podpisuje go kluczem prywatnym
    \item Zwraca pełny podpis (dokument|hash|podpis|klucz\_publiczny)
    \item Do weryfikacji użytkownik wkleja cały podpis i klika "Weryfikuj podpis"
    \item System sprawdza integralność dokumentu i autentyczność podpisu
    \item Wyświetla wynik: PODPIS WAŻNY lub PODPIS NIEWAŻNY
\end{enumerate}
...
\begin{lstlisting}[language=TypeScript, caption={Fragmenty klasy DigitalSignature}]
export default class DigitalSignature extends CryptographicAlgorithm {
  ...
  private E: bigint = 17n; // Wykładnik publiczny
